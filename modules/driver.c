...
/* $Id: driver.c,v 1.13 2000/03/08 21:45:26 alex Exp $ */
...
/*
 * We could redefine MOD_DEV because of a lack of braces in its definition
 * in /sys/sys/lkm.h that generates a warning with gcc -Wall.
...
static void CallbackTestSlrvLat(caddr_t param, int size)
{ 
  (*(int *) param)++;
  wakeup(param);
}
...
/*     log(LOG_DEBUG, "dev_ioctl\n"); */
  ...
  case HSLTESTSLRV:
    ...
    /* dest : noeud 1; channel : 3; 1 page de 1 octet */
    res = slrpv_send(1, 3, *(caddr_t *) data, 5000, NULL, NULL, p);
    return res;
    /* NOTREACHED */
    break;
...
res = slrpv_recv(0, 3, *(caddr_t *) data, 5000, NULL, NULL, p);
  ...
  case HSLTEST2SLRV:
...
#ifdef DEBUG_HSL
      log(LOG_DEBUG, "IOCTL HSLTEST2SLRV\n");
#endif
    ...
    while(!test2slrv_status) {
      res = tsleep(&test2slrv_status, HSLPRI|PCATCH, "DATASENT", 0);
      if (res) {
	log(LOG_ERR, "tsleep error");
	return res;
      }
    }
    test2slrv_status--;
...
#ifdef DEBUG_HSL
    log(LOG_DEBUG, "IOCTL HSLTEST2SLRV2\n");
#endif
...
/*A REMETTRE : (test loopback)     res = slrpv_recv(0, 3, *(caddr_t *) data, 5000, NULL, NULL, p); */
    ...
    if (res) {
      log(LOG_DEBUG," slrpv_recv error\n");
      return res;
    }
    ...
    while(!test2slrv_status) {
      res = tsleep(&test2slrv_status, HSLPRI|PCATCH, "DATARECV", 0);
      if (res) {
	log(LOG_ERR, "tsleep error");
	return res;
      }
  ...
  case HSLTESTSLRVLAT:
...
#ifdef DEBUG_HSL
    log(LOG_DEBUG, "IOCTL HSLTESTSLRVLAT\n");
#endif

    status = 0;
    lat_cpt++;
    rs = (latenceslr_t *) data;
    res = slrpv_send(rs->dest, 3, rs->data, rs->size, CallbackTestSlrvLat, (caddr_t) &status, p);
    if (res != 0)
      {
    ...
    ss=splhigh();
 /*   log(LOG_DEBUG,"Attendre l'envoi %d\n",lat_cpt);*/
    while(!status) tsleep(&status, HSLPRI|PCATCH, "DATASENT", 0);
    status--;
    splx(ss);
/*    log(LOG_DEBUG,"Fin de l'attente\n");*/
    return res;
  ...
  case HSLTESTSLRV2LAT:
    TRY("HSLTESTSLRV2LAT")

#ifdef DEBUG_HSL
    log(LOG_DEBUG, "IOCTL HSLTESTSLRV2LAT\n");
#endif

    status = 0;
    lat_cpt++;
    rs = (latenceslr_t *) data;
    res = slrpv_recv(rs->dest, 3, rs->data, rs->size, (void (*)(caddr_t)) CallbackTestSlrvLat, (caddr_t) &status, p);
    if (res != 0)
      {
    ...
    ss = splhigh();
/*    log(LOG_DEBUG,"Attendre la reception %d\n",lat_cpt); */
    while(!status) tsleep(&status, HSLPRI|PCATCH,"DATARECV", 0);
    status--;
    splx(ss);
/*    log(LOG_DEBUG,"Fin de l'attente\n");*/
    return res;
  ...
  case HSLSENDRANDOMPAGE:

    TRY("HSLSENDRANDOMPAGE")

#ifdef DEBUG_HSL
    log(LOG_DEBUG, "IOCTL HSLSENDRANDOMPAGE\n");
#endif

    {
      ...
      mi_t mi;
      lpe_entry_t entry;
      lpe_entry_t *lpe_current_p;
      u_long v0;
      int cpt_bcl;
      int cpt_msgs;
      int nb_msgs;

      v0 = ((sendrandompageinfo_t *) data)->v0;

      log(LOG_ERR,
	  "AVOID_LINK_ERROR: LPE_CURRENT index = 0x%x ; v0 = 0x%lx\n",
	  ((lpe_entry_t *)
	   (hsl_conf_read(minor(dev), PCIDDC_LPE_CURRENT) -
	    (u_long) put_info[minor(dev)].hsl_contig_space_phys +
	    (u_long) put_info[minor(dev)].hsl_contig_space)) -
	  (lpe_entry_t *) put_info[minor(dev)].hsl_lpe,
	  v0);

      s = splhigh();

      entry = ((sendrandompageinfo_t *) data)->entry;
	...
	return ENOMEM;
      ...
      /* S'assurer qu'il y a au moins 2 messages à la suite
	 de celui numéroté v0 */
      ...
      entry.routing_part = put_get_node(minor(dev)) ? 0 : 1;
      entry.PLSA = (caddr_t) vtophys(&wired_trash_data);

      res = _put_add_entry(minor(dev), &entry);
      if (res == EAGAIN)
	panic("AVOID_LINK_ERROR: LPE full\n");
      if (res) {
	splx(s);
	return res;
      }
	...
	return res;
      ...
      /* Insérer un marqueur dans le flux */

      entry.control = ((sendrandompageinfo_t *) data)->entry.control;
      entry.control |= mi + 5;
	...
	return res;
      ...
      /* Rechercher la 1ère entrée de LPE qui suit celles étiquetées v0 */

      nb_msgs = 0;

      lpe_current_p = (lpe_entry_t *)
	(hsl_conf_read(minor(dev), PCIDDC_LPE_CURRENT) -
	 (u_long) put_info[minor(dev)].hsl_contig_space_phys +
	 (u_long) put_info[minor(dev)].hsl_contig_space);

      cpt_bcl = 0;
      while (((lpe_current_p->control>>8) & 0xffff) != v0) {
	if (lpe_current_p-- == (lpe_entry_t *) put_info[minor(dev)].hsl_lpe)
	  lpe_current_p = ((lpe_entry_t *) put_info[minor(dev)].hsl_lpe) +
	    HSL_LPE_SIZE - 1;
	if (++cpt_bcl >= 256)
	  panic("AVOID_LINK_ERROR: can't find LPE entry\n");
      }

      cpt_bcl = 0;
      while (((lpe_current_p->control>>8) & 0xffff) == v0) {
	if (++lpe_current_p == 
	    (((lpe_entry_t *) put_info[minor(dev)].hsl_lpe) + HSL_LPE_SIZE))
	  lpe_current_p = (lpe_entry_t *) put_info[minor(dev)].hsl_lpe;
	if (++cpt_bcl >= 256)
	  panic("AVOID_LINK_ERROR: message too big\n");
      }

      /************************************************************/

      /* Renvoyer les 2 messages potentionnellement mal transmis.  On
      ne les re-signale pas à l'émission (ils ne l'ont pas encore été,
      mais ils le seront car ces entrées qu'on injecte dans la LPE y
      sont déjà présentes).  On les signale en réception car les
      entrées de LMI associées, éventuellement déjà présentes, vont
      être ignorées en réception. Bref, il y a 2 fois deux messages
      dans la LPE. La première série sert à la signalisation en
      émission, la seconde à la signalisation en réception. Ca veut
      notamment dire que pour que ce patch fonctionne, il faut
      impérativement que tous les messages soient signalés dans la LMI
      (mais par forcément par interruption). */

      for (cpt_msgs = 0; cpt_msgs < 2; ) {
	u_long old_control = lpe_current_p->control;
	lpe_current_p->control &= ~NOS_MASK;
	if (LMI_ENABLE_ISSET(old_control))
	  lpe_current_p->control |= NOR_MASK;
	avoid_MI_safe = TRUE;
	res = _put_add_entry(minor(dev), lpe_current_p);
	avoid_MI_safe = FALSE;
	lpe_current_p->control = old_control;
	if (res == EAGAIN)
	  panic("AVOID_LINK_ERROR: LPE full\n");
	if (res) {
	  splx(s);
	...
	if (LMI_ENABLE_ISSET(lpe_current_p->control))
	  cpt_msgs++;
...
#ifdef AVOID_LINK_ERROR
      if (avoid_stage) {
	((lpe_entry_t *) data)->PRSA = NULL;
	((lpe_entry_t *) data)->PLSA = (caddr_t) vtophys(&wired_trash_data);
	((lpe_entry_t *) data)->control = avoid_v0;
	splx(s);
	return 0;
      }
#endif
...
*/
  ...
  case DUMPCHANNELSTATE:
	...
	case CHAN_OPEN:
	  ...
	  log(LOG_DEBUG,
	      "channel[%d][%d] state OPENED - proto %s - classname 0x%x (uid:%d)\n",
	      i, j,
	      (char *) protocol_names[channel_protocol[i][j]],
  ...
  case DUMPINTERNALTABLES:
    ...
    for (i = 0; i < MAX_NODES - 1; i++)
      for (j = 0; j < RECEIVED_RECEIVE_SIZE; j++)
...
#else
lkm_hsldriver_unload(lkmtp, cmd)
#endif
     struct lkm_table *lkmtp;
     int cmd;
{
               ...
               void *arg;
{
  ...
  switch (cmd) {
  case MOD_LOAD:
    printf("Loaded hsl driver (KLD)\n");
    return lkm_hsldriver_load(NULL, 0);
    break;

  case MOD_UNLOAD:
    printf("Unloaded hsl driver (KLD)\n");
    return lkm_hsldriver_unload(NULL, 0);
    break;
...
CDEV_MODULE(hsldriver_mod, DEVMAJOR, cdev, hsldriver_load, 0);
